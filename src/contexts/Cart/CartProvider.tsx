...
const CartProvider = ({ children }: { children: JSX.Element }) => {
            ...
            if (add) {
                ...
                for (const item of newCartData.itemsOrdered) {
                    if (item.id === productId) {
                        ...
                        if (item.quantity === 0) {
                            newCartData.itemsOrdered = newCartData.itemsOrdered.filter((item) => item.id !== productId);
                        }
                    }
                }
            }
    ...
    return (
        <CartContext.Provider value={{ cartData, categoryClicked, handleCategoryClicked, handleCart, clearCart }}>
            {children}
